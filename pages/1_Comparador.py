...
for col, nome, m in [(col1, f"{T['carteira_a']} — {nome_a}", ma),
    ...
    with col:
        ...
        st.markdown(cartao(
            T["drawdown"], f"{m['dd']}%",
            euros_str=f"€{perda_dd:,.0f} {T.get('queda_maxima', 'de queda máxima')}",
            euros_cor="#F44336"
        ), unsafe_allow_html=True)
        st.markdown(cartao(T["sharpe"], f"{m['sharpe']}"), unsafe_allow_html=True)
        st.markdown(cartao(
            T["valor_final"], f"€{val_final:,.0f}",
            euros_str=f"{'+' if ganho>=0 else ''}€{ganho:,.0f} {T.get('de_ganho', 'de ganho')}",
            euros_cor=cor(ganho)
        ), unsafe_allow_html=True)
...
st.caption(T.get("caption_base100", "💡 **Base 100:** todos os ativos começam no mesmo ponto para comparar performance relativa. Ex: um valor de 180 significa que o ativo subiu 80% desde o início do período."))
